...
typedef struct multiple_arg
{
    int connfd;
    char hostname[MAXLINE], port[MAXLINE];
} MA;

void doit(MA *client_info);
...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        ...
        MA *temp_ma = (MA *)Malloc(sizeof(MA));
        //클라이언트주소 크기
        temp_ma->connfd = Accept(listenfd, (SA *)&clientaddr, //클라이언트의 연결을 기다리고 받아주는 것 -> 연결식별자가 리턴( 0보다 크거나 같음 ) / -1 출력되면 에러
                           &clientlen);                 // line:netp:tiny:accept

        Getnameinfo((SA *)&clientaddr, &clientlen, hostname, MAXLINE, port, MAXLINE,
                    0);

        strcpy(temp_ma->hostname, hostname);
        strcpy(temp_ma->port, port);

        // Getnameinfo((SA *)&clientaddr, clientlen, hostname, MAXLINE, port, MAXLINE,
        //             0);
        Pthread_create(&tid, NULL, thread, temp_ma);
        printf("%d", tid);
        // 리턴값이 0 / 잘 안 되면 에러코드
        // 클라이언트 Ip와 port 출력
...
void doit(MA *client_info)
{
    char buf[MAXLINE], method[MAXLINE], uri[MAXLINE], url[MAXLINE], version[MAXLINE], filename[MAXLINE], host[MAXLINE], port[MAXLINE];
    //예를 들어 uri는 파일이름과 인자, 버젼은 http 1.0 / 1.1
    char rebuf[MAX_OBJECT_SIZE]; // cgi인자는 ?뒤에 나오는 것으로 &로 구분
    rio_t client_rio, pts_rio;
    int fd = client_info->connfd;
    strcpy(host, client_info->hostname);
    strcpy(port, client_info->port);
